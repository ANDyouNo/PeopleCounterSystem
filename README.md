...
│   │   └── notifier_controller.py  # UDP-оповещатель посетителей (ESP32-C3)
...
├── esp32c3/
│   └── visitor_notifier/
│       └── visitor_notifier.ino      # Зуммер, батарея и 3 адресных LED
...
| `notifier_esp_enabled` | true | перезапуск | Включить оповещатель посетителей ESP32-C3 |
| `notifier_delay_off` | 10 | на лету | Задержка выключения индикатора людей после потери детекции (сек) |
...
### Visitor Notifier (`esp32c3/visitor_notifier/`)

Оповещатель на ESP32-C3 подаёт заметный трёхтональный сигнал один раз при
появлении людей и короткий сигнал при включении. Задержка
`notifier_delay_off` удерживает индикатор присутствия включённым при
кратковременной потере детекции. Он контролирует связь с программой ПК по UDP-heartbeat:
при потере heartbeat звучит предупреждение раз в 3 секунды, а второй LED
мигает красным. Для Li-ion 1S шкала первого LED: зелёный (100–90 %),
салатовый (89–75 %), жёлтый (74–50 %), оранжевый (49–25 %),
красно-оранжевый (24–20 %); при 19–10 % он мигает и напоминает раз в
10 минут, при 9–0 % мигает и напоминает раз в минуту.

| Подключение | ESP32-C3 |
|---|---|
| Buzzer TFM-51 | GPIO5 |
| Батарея через делитель 100 kΩ + 100 kΩ | GPIO4 (ADC) |
| Адресная LED-лента DIN | GPIO7 |

Первый, второй и третий пиксели ленты показывают соответственно батарею,
связь с ПК и присутствие людей. Текущая яркость задана в прошивке как
`LED_BRIGHTNESS = 32` (примерно 13 %).

ESP анонсирует напряжение батареи строкой `PCOUNTER_NOTIFIER:<mV>` каждые
5 секунд на порт 4215. Сервер отправляет команды на порт 4214; вручную их
посылать не требуется. Для прошивки нужна библиотека **Adafruit NeoPixel**.
Для точного измерения с делителем 100 kΩ + 100 kΩ желательно поставить
конденсатор 0.1 µF между GPIO4 и GND рядом с ESP32-C3.
